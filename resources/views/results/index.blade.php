...
<x-guest-layout :maxWidth="$maxWidth">
    <div class="py-4">
        <div class="flex justify-between items-end mb-8 border-b border-slate-100 pb-4">
            <div>
                <h1 class="text-2xl font-black text-slate-900 tracking-tighter">win / <span class="text-amber-500">history</span></h1>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($draws as $draw)
                <div class="bg-white rounded-2xl overflow-hidden border border-slate-100 hover:border-amber-200 transition duration-300 group">
                    <div class="p-5">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="font-black text-slate-900 tracking-tight leading-tight">{{ $draw->title }}</h3>
                            <span class="text-[9px] font-black text-slate-400 bg-slate-50 px-2 py-0.5 rounded border border-slate-100">#{{ str_pad($draw->id, 3, '0', STR_PAD_LEFT) }}</span>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-6">
                            <div class="bg-slate-50 p-3 rounded-xl border border-slate-50">
                                ...
                                <span class="text-xs font-bold text-slate-900">${{ number_format($draw->total_sales, 2) }}</span>
                            </div>
                            <div class="bg-amber-50/50 p-3 rounded-xl border border-amber-100/50">
                                <span class="block text-[8px] font-black text-amber-500 uppercase tracking-widest mb-1">Prize Pool</span>
                                <span class="text-xs font-bold text-amber-600">${{ number_format($draw->prize_pool_total, 2) }}</span>
                            </div>
                        </div>

                        <div class="flex items-center justify-between">
                             <div class="text-[9px] text-slate-400 font-bold italic">{{ $draw->draw_time->format('M d, Y') }}</div>
                             <a href="{{ route('results.show', $draw->id) }}" class="text-[9px] font-black uppercase tracking-widest text-slate-900 group-hover:text-amber-500 transition-colors flex items-center">
