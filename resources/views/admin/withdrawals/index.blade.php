<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 py-8">
            <div>
                <nav class="flex text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 space-x-2">
                    <a href="{{ route('dashboard') }}" class="hover:text-amber-500 transition">admin</a>
                    <span>/</span>
                    <span class="text-amber-500">withdrawals</span>
                </nav>
    ...
    <div class="pb-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if(session('success'))
                <div class="mb-8 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-emerald-600 font-bold flex items-center space-x-3 animate-fade-in">
            ...
            @if(session('error'))
                <div class="mb-8 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl text-red-600 font-bold flex items-center space-x-3 animate-fade-in">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
                <div class="stats-card">
                    ...
                    <div class="stats-value text-amber-500 italic">{{ $requests->where('status', 'pending')->count() }}</div>
            ...
            <div class="pro-table-container overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr>
                                <th class="pro-table-header">Personnel & Node</th>
                                <th class="pro-table-header">Valuation</th>
                                <th class="pro-table-header">Protocol & Terminal</th>
                                <th class="pro-table-header">Status</th>
                                <th class="pro-table-header">Timestamp</th>
                                <th class="pro-table-header text-right">Administrative Decision</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="pro-table-row">
                                    <td class="px-8 py-6">
                                        <div class="flex items-center space-x-4">
                                            <div class="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center text-[10px] font-black text-amber-400">
                                                {{ strtoupper(substr($req->user->name, 0, 2)) }}
                                            </div>
                                            <div>
                                                <div class="font-black text-slate-900 tracking-tight">{{ $req->user->name }}</div>
                                                <div class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{{ $req->user->email }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="text-sm font-black text-slate-900 tracking-tighter">${{ number_format($req->amount, 2) }}</div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="text-[10px] font-black text-slate-900 uppercase tracking-widest">{{ $req->payment_method }}</div>
                                    ...
                                    <td class="px-8 py-6">
                                        <span class="badge-dot {{ $req->status === 'pending' ? 'badge-dot-live' : ($req->status === 'approved' ? 'badge-dot-live' : 'badge-dot-ended') }}">
                                            <span class="dot {{ $req->status === 'pending' ? 'dot-live !bg-amber-400' : ($req->status === 'approved' ? 'dot-live' : 'dot-ended') }}"></span>
                                        ...
                                        @if($req->status === 'pending')
                                            <div class="flex items-center justify-end space-x-3" x-data="{ showReject: false }">
                                                <form action="{{ route('admin.withdrawals.approve', $req->id) }}" method="POST" onsubmit="return confirm('Authorize Capital Outflow?');">
                                                    @csrf
                                                    <button type="submit" class="btn-pro-action !bg-emerald-500 !text-white hover:!bg-emerald-600">
                                                        Authorize
                                                    </button>
                                                </form>
                                                <button @click="showReject = true" class="btn-pro-danger">
                                                    Terminate
                                                </button>
                                                ...
                                                <template x-if="showReject">
                                                    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/20 backdrop-blur-sm">
                                                        <div class="bg-white rounded-[2rem] p-8 shadow-2xl max-w-md w-full border border-slate-100 animate-fade-in-up">
                                                            ...
                                                            <form action="{{ route('admin.withdrawals.reject', $req->id) }}" method="POST" class="space-y-4">
                                                                @csrf
                                                                <textarea name="admin_notes" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-amber-400 transition text-xs" rows="3" placeholder="Rejection reasoning..."></textarea>
                                                                <div class="flex items-center space-x-4">
                                                                    <button type="submit" class="btn-pro-danger flex-1 !rounded-xl uppercase tracking-widest text-[10px]">Commit Rejection</button>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-8 py-12 text-center">
                                        <div class="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">No Outflow Requests Logged</div>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
