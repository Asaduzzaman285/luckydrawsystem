                                ...
                                <tr class="hover:bg-blue-50/30 transition">
                                    <td class="px-8 py-6">
                                        <div class="flex items-center space-x-4">
                                            <div class="w-10 h-10 bg-blue-50 rounded-xl flex items-center justify-center text-xs font-black text-blue-600 border border-blue-100 italic">
                                                {{ strtoupper(substr($user->name, 0, 2)) }}
                                            </div>
                                            <div>
                                                <div class="font-black text-slate-900 tracking-tight italic">{{ $user->name }}</div>
                                                <div class="flex items-center space-x-2 mt-1">
                                                    ...
                                                    <span class="text-[10px] text-blue-600/80 font-black italic">{{ $user->phone }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-8 py-6 text-right">
                                        <div class="flex items-center justify-end space-x-3">
                                            ...
                                            <button @click="creditModal = true; selectedUser = {{ $user->id }}; selectedUserName = '{{ $user->name }}'" class="btn-pro-action transition">
                                                <span class="mr-1.5">+</span> Fund
                                            </button>
                                            @endif
                                            <a href="{{ route('users.edit', $user->id) }}" class="btn-pro-secondary hover:!bg-blue-600 hover:!text-white hover:!border-blue-600 transition">Configure</a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                ...
                <div x-show="createModal" 
                    ...
                    <div class="bg-white w-full max-w-2xl rounded-[3rem] p-12 shadow-2xl relative border border-white">
                        ...
                        <div class="mb-10 text-center text-slate-900">
                            ...
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">New Distribution Protocol Initiation</p>
                        </div>
                        <form action="{{ route('users.store') }}" method="POST" class="space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    ...
                                    <input type="text" name="name" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-blue-600 transition" placeholder="e.g. John Agent" required>
                                </div>
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Phone Number</label>
                                    <input type="text" name="phone" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-blue-600 transition" placeholder="e.g. 01711223344" required>
                                </div>
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email Address (Optional)</label>
                                    <input type="email" name="email" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-blue-600 transition" placeholder="e.g. john@luckydraw.com">
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Password</label>
                                    <input type="password" name="password" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-blue-600" required>
                                </div>
                                <div class="space-y-2">
                                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Confirm</label>
                                    <input type="password" name="password_confirmation" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-blue-600" required>
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="{ selectedRole: 'agent' }">
                                <div class="space-y-2">
                                    ...
                                    <select name="role" x-model="selectedRole" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-blue-600 transition text-sm">
                                ...
                                <div class="space-y-2" x-show="selectedRole === 'agent'">
                                    ...
                                    <select id="modal_district_id" name="district_id" onchange="fetchModalUpazillas(this.value)" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-blue-600 transition text-sm">
                                ...
                                <div class="space-y-2" x-show="selectedRole === 'agent'">
                                    ...
                                    <select id="modal_upazilla_id" name="upazilla_id" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-blue-600 transition text-sm">
                                        <option value="">Select Upazilla</option>
                                    </select>
                                </div>
                            </div>
                            <button type="submit" class="w-full bg-blue-600 text-white font-black py-5 rounded-2xl shadow-xl shadow-blue-900/20 hover:scale-[1.02] transition uppercase tracking-widest text-xs italic">
                                Authorize Account
                            </button>
                        </form>
                        <script>
                ...
                <div x-show="creditModal" 
                    ...
                    <div class="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl relative border border-white">
                        ...
                        <div class="mb-8 text-slate-900">
                            ...
                            <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Dispensing credit to: <span class="text-blue-600 font-black italic" x-text="selectedUserName"></span></p>
                        </div>
                        <form :action="'{{ url('/users') }}/' + selectedUser + '/credit'" method="POST" class="space-y-6">
                            @csrf
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Funding Amount</label>
                                <div class="relative">
                                    <input type="number" step="0.01" name="amount" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-5 px-6 text-xl font-black text-slate-900 focus:ring-blue-600" placeholder="0.00" required>
                                    <span class="absolute right-6 top-1/2 -translate-y-1/2 text-xs font-black text-slate-400">BDT</span>
                                </div>
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Reference / Note</label>
                                <input type="text" name="reference" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-4 px-6 font-bold text-slate-900 focus:ring-blue-600" placeholder="e.g. Batch #102">
                            </div>
                            <button type="submit" class="w-full bg-blue-600 text-white font-black py-5 rounded-2xl shadow-xl shadow-blue-500/20 hover:scale-[1.02] transition duration-300 uppercase tracking-widest text-xs italic">
                                Confirm Funding
                            </button>
                        </form>
                    </div>
