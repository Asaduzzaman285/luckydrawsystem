            ...
            <div class="relative z-10">
                <div class="flex items-center space-x-4 mb-6">
                    ...
                    <div>
                        ...
                        <h1 class="text-2xl font-black text-white tracking-tight italic">Transaction Reports</h1>
                    </div>
                </div>

                <!-- Filters -->
                <div class="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide">
                    <a href="{{ route('agent.reports') }}" class="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest italic whitespace-nowrap {{ !request('type') ? 'bg-white text-blue-600' : 'bg-white/10 text-white hover:bg-white/20' }}">All</a>
                    <a href="{{ route('agent.reports', ['type' => 'deposit']) }}" class="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest italic whitespace-nowrap {{ request('type') == 'deposit' ? 'bg-white text-blue-600' : 'bg-white/10 text-white hover:bg-white/20' }}">Deposits</a>
                    <a href="{{ route('agent.reports', ['type' => 'withdrawal']) }}" class="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest italic whitespace-nowrap {{ request('type') == 'withdrawal' ? 'bg-white text-blue-600' : 'bg-white/10 text-white hover:bg-white/20' }}">Withdrawals</a>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 md:px-8 max-w-7xl mx-auto">
            <div class="bg-white rounded-[2rem] overflow-hidden shadow-xl shadow-blue-900/5 border border-white">
                <div class="px-4 md:px-8 py-5 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                    <h3 class="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em] italic">Processed Transactions / <span class="text-blue-600 font-bold italic">{{ $transactions->total() }} Records</span></h3>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            @forelse($transactions as $txn)
                                <tr class="hover:bg-blue-50/30 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-xs font-black text-slate-900 tracking-tight italic">{{ $txn->user->name }}</div>
