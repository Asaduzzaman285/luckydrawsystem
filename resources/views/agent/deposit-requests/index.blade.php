    ...
    <div class="min-h-screen bg-var(--background) pb-24">
            ...
            <div class="relative z-10">
                <div class="flex items-center space-x-4 mb-6">
                    ...
                    <div>
                        ...
                        <h1 class="text-2xl font-black text-white tracking-tight italic">Pending Deposit Requests</h1>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 md:px-8 max-w-7xl mx-auto">
            ...
            <div class="bg-blue-50 border-l-4 border-blue-600 p-6 rounded-2xl mb-8">
                <h3 class="text-xs font-black text-blue-900 uppercase tracking-widest italic mb-2">Instructions for Agents</h3>
                <p class="text-[11px] text-blue-800/80 font-medium leading-relaxed">
                    Verify the Transaction ID in your bKash or Nagad app before approving. Once approved, the amount is added to the user's wallet.
                </p>
            </div>
            ...
            <div class="bg-white rounded-[2rem] overflow-hidden shadow-xl shadow-blue-900/5 border border-white">
                <div class="px-4 md:px-8 py-5 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                    <h3 class="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em] italic">Deposit Requests / <span class="text-blue-600 font-bold italic">{{ $requests->count() }} Total</span></h3>
                </div>
                                    ...
                                    <td class="px-6 py-4 text-right">
                                        @if($request->status == 'pending')
                                            <div class="flex justify-end space-x-2" x-data="{ rejectModal: false }">
                                                <form action="{{ route('agent.deposit-requests.approve', $request->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to approve this deposit? The amount will be added to the customer\'s wallet.');">
                                                    @csrf
                                                ...
                                                <div x-show="rejectModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" style="display: none;">
                                                    <div @click.away="rejectModal = false" class="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl">
                                                        ...
                                                        <form action="{{ route('agent.deposit-requests.reject', $request->id) }}" method="POST">
                                                            @csrf
                                                            <div class="space-y-4 text-left">
                                                                <div>
                                                                    ...
                                                                    <input type="text" name="rejection_reason" required placeholder="e.g. Invalid Transaction ID" class="w-full bg-slate-50 border-slate-200 rounded-xl py-3 px-4 text-xs font-black text-slate-900">
                                                                </div>
