    ...
    <div class="min-h-screen bg-var(--background) pb-24" x-data="{ amount: 500 }">
        ...
        <div class="bg-blue-600 rounded-b-[3rem] p-8 pt-12 shadow-2xl relative overflow-hidden mb-8">
            ...
            <div class="relative z-10">
                <div class="flex items-center space-x-4 mb-6">
                    <a href="{{ route('agent.dashboard') }}" class="w-10 h-10 bg-white/10 rounded-2xl flex items-center justify-center text-white backdrop-blur-sm hover:bg-white/20 transition">
                        <span class="text-lg">←</span>
                    </a>
                    <div>
                        <div class="text-[10px] font-black text-white/60 uppercase tracking-widest italic">Agent Console</div>
                        <h1 class="text-2xl font-black text-white tracking-tight italic">Request Wallet Top-Up</h1>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="px-4 md:px-8 max-w-2xl mx-auto">
            ...
            <div class="bg-blue-50 border-l-4 border-blue-600 p-6 rounded-2xl mb-8">
                <h3 class="text-xs font-black text-blue-900 uppercase tracking-widest italic mb-2">Step 1: Send Money to Admin</h3>
                <p class="text-[11px] text-blue-800/80 font-medium leading-relaxed">
                    Send the desired amount to the Admin via bKash or Nagad, then submit the transaction details below for verification.
                </p>
                @if($admin)
                <div class="mt-4 bg-white p-4 rounded-xl shadow-sm border border-blue-100">
                    <div class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic mb-1">Admin Contact</div>
                    <div class="flex justify-between items-center">
                        <div class="text-sm font-black text-slate-900 italic">{{ $admin->name }}</div>
                        <div class="text-xs font-black text-blue-600 italic tracking-tighter">{{ $admin->phone ?? 'Contact Admin' }}</div>
                    </div>
                </div>
                @endif
            </div>
            ...
            <!-- Form -->
            <form action="{{ route('agent.admin-deposit.store') }}" method="POST" class="space-y-6 bg-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-blue-900/5 border border-white">
                @csrf
                ...
                <!-- Amount -->
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest italic block">Amount (৳)</label>
                    <div class="grid grid-cols-4 gap-2 mb-3">
                        ...
                        <button type="button" @click="amount = 2000" :class="amount == 2000 ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'" class="py-3 rounded-xl text-xs font-black italic transition">2000</button>
                        <button type="button" @click="amount = 5000" :class="amount == 5000 ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'" class="py-3 rounded-xl text-xs font-black italic transition">5000</button>
                        <button type="button" @click="amount = 10000" :class="amount == 10000 ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'" class="py-3 rounded-xl text-xs font-black italic transition">10000</button>
                    </div>
                    <div class="relative">
                ...
                <!-- Payment Method -->
                <div class="space-y-3">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest italic block">Payment Method</label>
                ...
                <div class="space-y-3">
                    ...
                    <p class="text-[9px] font-medium text-slate-400 italic">Enter the exact Transaction ID from bKash/Nagad after sending the money.</p>
